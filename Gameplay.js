...
function update() {
    ...
    //Сбор монеток
    for(let i=0;i<bonusesList.length;i++){
        if(Math.abs(bonusesList[i].x + 25 - player.x - 15) < 50 && Math.abs(bonusesList[i].y - player.y) < 25){
            bonusesList = bonusesList.slice(0,i).concat(bonusesList.slice(i+1,bonusesList.length));
            gameStatistic.score++;
            break;
        }
...
function render(){
    ...
    document.getElementById('statusPanel').innerHTML = 'Score: ' + gameStatistic.score +'; Hp: ' + gameStatistic.hp;
